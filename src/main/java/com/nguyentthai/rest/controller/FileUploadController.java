...
import com.nguyentthai.rest.exception.StorageFileNotFoundException;
...
//    }
    ...
    public String listUploadedFiles() {
        ...
        return storageService.loadAll()
                .map(path -> MvcUriComponentsBuilder.fromMethodName(FileUploadController.class,
                        "serveFile", path.getFileName().toString()).build().toString()).toString();
    }
    ...
    @RequestMapping(value = "/loadimage/{mobileImageUrl:.+}", produces = MediaType.IMAGE_JPEG_VALUE, method = RequestMethod.GET)
    public @ResponseBody
    ResponseEntity<Resource> downloadFile(@PathVariable String mobileImageUrl) {
//        String fromFile = imageWhere + mobileImageUrl;
//        File file = FileUtils.getFile(fromFile);
//        FileSystemResource fileSystemResource = new FileSystemResource(file);

        Resource file = storageService.loadAsResource(mobileImageUrl);
